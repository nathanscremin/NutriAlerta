...
def agregar_para_modelo(
    df_limpo: pd.DataFrame,
    df_ubs_caminho: str,
    base_existente_caminho: str,
) -> None:
    """
    ...
    try:
        ...
        df_ubs["cnes"] = df_ubs["cnes"].astype(str).str.strip().str.split(".").str[0]
    ...
    # Garantir que a coluna codigo_cnes seja tratada de forma robusta e limpa
    df_limpo["codigo_cnes"] = (
        df_limpo["codigo_cnes"]
        .astype(str)
        .str.strip()
        .str.split(".")
    ...
    for faixa, cond in [
        ...
        for (cnes, ano), group in grouped:
            # Pular grupos de CNES nulos ou vazios
            if not cnes or pd.isna(cnes) or str(cnes).lower() in ("nan", "none", ""):
                continue
            ...
            row = {
                ...
                "IBGE": (
                    int(group["codigo_municipio"].iloc[0])
                    if "codigo_municipio" in group.columns
                    else 354390
                ),
                "Municipio": (
                    group["municipio"].iloc[0]
                    if "municipio" in group.columns
                    else "RIO CLARO"
                ),
                "CNES": cnes,
            ...
            if faixa in ("0 a 5 anos", "6 a 10 anos"):
                ...
                contagens = (
                    group["peso_x_idade"].value_counts()
                    if "peso_x_idade" in group.columns
                    else pd.Series()
                )
                
                muito_baixo = float(
                    contagens.get("MUITO BAIXO PESO PARA A IDADE", 0.0)
                    or contagens.filter(like="MUITO BAIXO").sum()
                )
                baixo = float(
                    contagens.get("BAIXO PESO PARA A IDADE", 0.0)
                    or contagens.filter(like="BAIXO PESO").sum()
                )
                adequado = float(
                    contagens.get("PESO ADEQUADO PARA A IDADE", 0.0)
                    or contagens.filter(like="PESO ADEQUADO").sum()
                )
                elevado = float(
                    contagens.get("PESO ELEVADO PARA A IDADE", 0.0)
                    or contagens.filter(like="PESO ELEVADO").sum()
                )
                
                total_validos = muito_baixo + adequado + baixo + elevado
                if total_validos > 0:
                ...
                contagens = (
                    group["adolescente_imc_x_idade"].value_counts()
                    if "adolescente_imc_x_idade" in group.columns
                    else pd.Series()
                )
                
                magreza_ac = float(
                    contagens.get("MAGREZA ACENTUADA", 0.0)
                    or contagens.filter(like="ACENTUADA").sum()
                )
                magreza = float(
                    contagens.get("MAGREZA", 0.0)
                    or contagens.filter(like="MAGREZA").sum() - magreza_ac
                )
                eutrofia = float(
                    contagens.get("EUTROFIA", 0.0)
                    or contagens.filter(like="EUTROFIA").sum()
                )
                sobrepeso = float(
                    contagens.get("SOBREPESO", 0.0)
                    or contagens.filter(like="SOBREPESO").sum()
                )
                obesidade = float(
                    contagens.get("OBESIDADE", 0.0)
                    or contagens.filter(like="OBESIDADE").sum()
                    - contagens.filter(like="GRAVE").sum()
                )
                obesidade_gr = float(
                    contagens.get("OBESIDADE GRAVE", 0.0)
                    or contagens.filter(like="GRAVE").sum()
                )
    ...
    for (cnes, ano), grp in df_novos_agregados.groupby(["CNES", "Ano"]):
        ...
        for _, r in grp.iterrows():
            ...
            if faixa in ("0 a 5 anos", "6 a 10 anos"):
                total_magreza += (
                    0.0
                    if pd.isna(r.get("Peso_Muito_Baixo_Quantidade"))
                    else r["Peso_Muito_Baixo_Quantidade"]
                ) + (
                    0.0
                    if pd.isna(r.get("Peso_Baixo_Quantidade"))
                    else r["Peso_Baixo_Quantidade"]
                )
                total_obesidade += (
                    0.0
                    if pd.isna(r.get("Peso_Elevado_Quantidade"))
                    else r["Peso_Elevado_Quantidade"]
                )
                total_eutrofia += (
                    0.0
                    if pd.isna(r.get("Peso_Adequado_Quantidade"))
                    else r["Peso_Adequado_Quantidade"]
                )
            elif faixa == "10 a 19 anos":
                total_magreza += (
                    0.0
                    if pd.isna(r.get("Magreza_Acentuada_Qtd"))
                    else r["Magreza_Acentuada_Qtd"]
                ) + (
                    0.0 if pd.isna(r.get("Magreza_Qtd")) else r["Magreza_Qtd"]
                )
                total_obesidade += (
                    0.0 if pd.isna(r.get("Obesidade_Qtd")) else r["Obesidade_Qtd"]
                ) + (
                    0.0
                    if pd.isna(r.get("Obesidade_Grave_Qtd"))
                    else r["Obesidade_Grave_Qtd"]
                )
                total_obesidade_grave += (
                    0.0
                    if pd.isna(r.get("Obesidade_Grave_Qtd"))
                    else r["Obesidade_Grave_Qtd"]
                )
                total_sobrepeso += (
                    0.0 if pd.isna(r.get("Sobrepeso_Qtd")) else r["Sobrepeso_Qtd"]
                )
                total_eutrofia += (
                    0.0 if pd.isna(r.get("Eutrofia_Qtd")) else r["Eutrofia_Qtd"]
                )
        ...
        row_unif = {
            ...
            "Magreza_Pct": (
                total_magreza / total_geral * 100 if total_geral > 0 else 0.0
            ),
            "Eutrofia_Qtd": total_eutrofia,
            "Eutrofia_Pct": (
                total_eutrofia / total_geral * 100 if total_geral > 0 else 0.0
            ),
            "Sobrepeso_Qtd": total_sobrepeso,
            "Sobrepeso_Pct": (
                total_sobrepeso / total_geral * 100 if total_geral > 0 else 0.0
            ),
            "Obesidade_Qtd": total_obesidade,
            "Obesidade_Pct": (
                total_obesidade / total_geral * 100 if total_geral > 0 else 0.0
            ),
            "Obesidade_Grave_Qtd": total_obesidade_grave,
            "Obesidade_Grave_Pct": (
                total_obesidade_grave / total_geral * 100 if total_geral > 0 else 0.0
            ),
        }
    ...
    if linhas_unificadas:
        ...
        df_novos_agregados = pd.concat(
            [df_novos_agregados, df_unificados], ignore_index=True
        )
        logger.info(
            "Linhas unificadas '0 a 18 anos' criadas: %d", len(df_unificados)
        )

    # 3. Upsert com a base existente
    if os.path.exists(base_existente_caminho):
        logger.info(
            "Mesclando dados novos com a base existente: '%s'",
            base_existente_caminho,
        )
        ...
        chaves_novas = set(
            zip(
                df_novos_agregados["CNES"],
                df_novos_agregados["Ano"],
                df_novos_agregados["Faixa_Etaria"],
            )
        )
        
        mask_existente_manter = df_existente.apply(
            lambda r: (str(r["CNES"]), int(r["Ano"]), str(r["Faixa_Etaria"]))
            not in chaves_novas,
            axis=1,
        )
        ...
        df_final = pd.concat(
            [df_existente_filtrado, df_novos_agregados], ignore_index=True
        )
    else:
        logger.info(
            "Criando nova base consolidada: '%s'", base_existente_caminho
        )
    ...
    ordem_colunas = [
        ...
        "CNES",
        "EAS",
        "Peso_Muito_Baixo_Quantidade",
        "Peso_Muito_Baixo_Porcentagem",
        "Peso_Baixo_Quantidade",
        "Peso_Baixo_Porcentagem",
        "Peso_Adequado_Quantidade",
        "Peso_Adequado_Porcentagem",
        "Peso_Elevado_Quantidade",
        "Peso_Elevado_Porcentagem",
        "Total",
        "Local",
        "Ano",
        "Faixa_Etaria",
        "Magreza_Acentuada_Qtd",
        "Magreza_Acentuada_Pct",
        "Magreza_Qtd",
        "Magreza_Pct",
        "Eutrofia_Qtd",
        "Eutrofia_Pct",
    ...
    df_final = df_final.sort_values(by=["CNES", "Ano", "Faixa_Etaria"]).reset_index(
        drop=True
    )
    
    os.makedirs(
        os.path.dirname(os.path.abspath(base_existente_caminho)), exist_ok=True
    )
    df_final.to_csv(base_existente_caminho, index=False)
    logger.info(
        "Base consolidada final atualizada com sucesso! Total de linhas: %d",
        len(df_final),
    )
