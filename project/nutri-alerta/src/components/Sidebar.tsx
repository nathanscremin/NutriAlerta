...
import { Activity, TrendingUp, Users, Stethoscope, Calendar, Map, Layers, ChevronLeft, Moon, Sun, ShieldCheck, Globe } from 'lucide-react';
      ...
      <div className="flex-1 p-4 space-y-5 overflow-y-auto scrollbar-thin">
        ...
        <div>
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-extrabold transition-all duration-300 mb-3 border ${
              indicador === 'global'
                ? 'bg-teal-550/10 dark:bg-teal-950/30 border-teal-200 dark:border-teal-900/60 text-teal-700 dark:text-teal-400 shadow-md shadow-teal-500/5'
                : 'bg-slate-50 dark:bg-zinc-800/20 border-transparent text-slate-600 dark:text-zinc-400 hover:bg-slate-100/50 dark:hover:bg-zinc-800/40 hover:text-slate-800 dark:hover:text-[#f5f5f7]'
            }`}
          >
            <Globe className={`w-3.5 h-3.5 shrink-0 transition-transform duration-700 ${indicador === 'global' ? 'rotate-180 text-teal-600 dark:text-teal-500' : 'text-slate-400 dark:text-zinc-500'}`} />
            <span>Mapa Global Integrado</span>
            {indicador === 'global' && (
              <span className="ml-auto text-[9px] font-black uppercase tracking-wider text-teal-600 dark:text-teal-400 bg-teal-100/50 dark:bg-teal-900/40 px-1.5 py-0.5 rounded">ativo</span>
            )}
          </button>
          ...
          <div className="space-y-1.5">
            {[
              ...
              { id: 'eutrofia',   label: 'Peso Adequado', color: 'text-emerald-500', dot: 'bg-emerald-500' },
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all duration-300 ${
                  indicador === id
                    ...
                    : indicador === 'global'
                      ? 'opacity-40 cursor-not-allowed text-slate-400 dark:text-zinc-600 border border-transparent'
        ...
        <div>
          ...
          <div className="space-y-2">
            <MetricRow icon={<ShieldCheck className="w-3.5 h-3.5 text-emerald-500" />} label="Peso adequado médio" value={avgEut} color="text-emerald-500" />
            <MetricRow icon={<TrendingUp className="w-3.5 h-3.5 text-red-500" />} label="Obesidade média" value={avgObs} color="text-red-500" />
            <MetricRow icon={<TrendingUp className="w-3.5 h-3.5 text-amber-500" />} label="Sobrepeso médio" value={avgSob} color="text-amber-500" />
