...
// Distribuição nutricional média de 2025 (Fonte: dados reais SISVAN)
...
export const ALL_POIS = [
  ...UNIDADES_SAUDE.map(u => ({ id: `saude-${u.nome}`, nome: u.nome, categoria: u.categoria, lat: u.lat, lon: u.lon, color: u.categoria === 'UBS' ? '#e74c3c' : u.categoria === 'Pronto-Atendimento' ? '#8e44ad' : u.categoria === 'Saúde Mental' ? '#f39c12' : '#c0392b' })),
  ...filteredExtracted.map((p, i) => ({ id: `extracted-${i}`, nome: p.nome === 'Desconhecido' ? p.categoria + ' (Mapeado)' : p.nome, categoria: p.categoria, lat: p.lat, lon: p.lon, color: p.color }))
];
...
export const getVoronoiGeoJSON = () => {
  const ubs_pts = UNIDADES_SAUDE.filter(p => p.categoria === 'UBS');
  
  // Bounding box expandida cobrindo toda a extensão municipal de Rio Claro
  const min_lon = -47.78;
  const max_lon = -47.45;
  const min_lat = -22.56;
  const max_lat = -22.24;
  ...
  const voronoi = delaunay.voronoi([min_lon, min_lat, max_lon, max_lat]);
  
  const features = ubs_pts.map((ubs, index) => {
    const cell = voronoi.cellPolygon(index);
    if (!cell) return null;
    
    // d3-delaunay gera um polígono de vértices exatos [lon, lat]
    const coords = [...cell];
    
    try {
      // Cria o polígono da célula para o Turf
      const cellPoly = polygon([coords]);
      
      // Intersecta com a borda do município de Rio Claro para recortar
      const clipped = intersect(featureCollection([cellPoly, rioClaroBoundary as any]));
      ...
      return {
        type: 'Feature',
        properties: {
          nome_bairro: ubs.nome,
          categoria: 'UBS Region'
        },
        geometry: clipped.geometry
      };
    } catch (err) {
      console.error(`Erro ao recortar célula para a UBS ${ubs.nome}:`, err);
      // Fallback: retorna a célula inteira do Voronoi em caso de falha de geometria
      return {
        type: 'Feature',
        properties: {
          nome_bairro: ubs.nome,
